...
async function carregarPedidos(){
  try{
    ...
    console.warn(err); usandoNuvem = false; loadLocal(); setStatus('Modo local • verifique o supabase.sql');
  }
  renderAll();
}
...
async function carregarProdutos(){
  ...
  try{
    ...
    saveProductsLocal();
  }catch(err){ console.warn('Produtos usando modo local:', err); }
  renderProdutos();
}
...
async function salvarProduto(nome){
  ...
  produtos.push(nome); produtos.sort((a,b)=>a.localeCompare(b)); saveProductsLocal(); renderProdutos();
  if(usandoNuvem){
    try{
      const res = await fetch(productsApi, { method:'POST', headers:headers(), body:JSON.stringify({ nome }) });
      if(!res.ok) throw new Error(await res.text());
    }catch(err){ console.warn(err); toast('Produto salvo localmente. Rode o supabase.sql para salvar na nuvem.'); }
  }
}
async function excluirProduto(nome){
  if(!confirm(`Excluir o produto "${nome}" das opções?`)) return;
  produtos = produtos.filter(p=>p !== nome); saveProductsLocal(); renderProdutos();
  if(usandoNuvem){
...
async function salvarListaMes(){
  ...
  if(!usandoNuvem){
    ...
    pedidos=[]; saveLocal(); saveHistLocal(); renderAll(); toast('Lista do mês salva.'); return;
  }
  ...
  pedidos=[]; saveLocal(); await carregarHistorico(); renderAll(); toast('Lista do mês salva e lista atual limpa.');
}

function renderAll(){ render(); renderProdutos(); renderHistorico(); }
function render(){
  const lista = $('listaPedidos'); if(!lista) return;
  ...
  if(!filtrados.length){ lista.appendChild($('emptyTemplate').content.cloneNode(true)); }
    ...
    const tr=document.createElement('tr'); if(p.checked) tr.classList.add('checked');
...
function renderResumo(){
  const box=$('resumoProdutos'); if(!box) return;
  box.innerHTML=''; if(!pedidos.length){box.innerHTML='<p class="empty">Nenhum produto para consolidar.</p>'; return;}
  const grupos={}; pedidos.forEach(p=>{const key=p.produto.trim(); if(!grupos[key]) grupos[key]={total:0,pessoas:[]}; grupos[key].total+=Number(p.quantidade); grupos[key].pessoas.push(`${p.funcionario}: ${p.quantidade}${p.checked?' ✓':''}`);});
  Object.entries(grupos).sort((a,b)=>a[0].localeCompare(b[0])).forEach(([produto,info])=>{const card=document.createElement('div'); card.className='card'; card.innerHTML=`<h4>${escapeHtml(produto)}</h4><div class="qty">${info.total}</div><small>Total solicitado</small><ul>${info.pessoas.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul>`; box.appendChild(card);});
}
function renderProdutos(){
  const datalist = $('produtosSugestoes'); if(datalist) datalist.innerHTML = produtos.map(p=>`<option value="${escapeHtml(p)}"></option>`).join('');
  const box = $('listaProdutos'); if(!box) return;
  if(!produtos.length){ box.innerHTML = '<p class="empty">Nenhum produto cadastrado.</p>'; return; }
  box.innerHTML = produtos.map(p=>`<div class="product-chip"><span>${escapeHtml(p)}</span><button type="button" data-remove-product="${escapeHtml(p)}">Excluir</button></div>`).join('');
}
function renderHistorico(){
  const box=$('historicoListas'); if(!box) return; box.innerHTML='';
  if(!historico.length){ box.innerHTML='<p class="empty">Nenhuma lista mensal salva ainda.</p>'; return; }
  historico.forEach(h=>{ const div=document.createElement('div'); div.className='history-card'; const resumo={}; (h.pedidos||[]).forEach(p=>{resumo[p.produto]=(resumo[p.produto]||0)+Number(p.quantidade)}); div.innerHTML=`<h4>Lista ${escapeHtml(h.mes)}</h4><div class="meta">Salva em ${fmtDate(h.createdAt)} • ${h.totalPedidos} pedidos • ${h.totalItens} itens</div><p>${escapeHtml(h.observacao)}</p><details><summary>Ver produtos e funcionários</summary><ul>${Object.entries(resumo).map(([prod,q])=>`<li><strong>${escapeHtml(prod)}</strong>: ${q}</li>`).join('')}</ul><hr><ul>${(h.pedidos||[]).map(p=>`<li>${escapeHtml(p.funcionario)} — ${escapeHtml(p.produto)} — ${p.quantidade}</li>`).join('')}</ul></details>`; box.appendChild(div); });
}
...
function openTab(tabName, btn){
  ...
  const target = $(`tab-${tabName}`); if(target) target.classList.add('show');
}
function initEvents(){
  $('mesReferencia').value=currentMonth();
  document.querySelectorAll('.nav[data-tab]').forEach(btn=>btn.addEventListener('click',()=>openTab(btn.dataset.tab, btn)));
  $('imprimirPdf')?.addEventListener('click',()=>window.print());
  $('recarregar')?.addEventListener('click',carregarPedidos);
  $('pedidoForm')?.addEventListener('submit',async(e)=>{ e.preventDefault(); const funcionario=$('funcionario').value.trim(); const produto=$('produto').value.trim(); const quantidade=Number($('quantidade').value); if(!funcionario||!produto||quantidade<1)return; const btn=e.target.querySelector('button[type="submit"]'); btn.disabled=true; btn.textContent='Salvando...'; try{ await criarPedido({funcionario,produto,quantidade}); if(!produtos.some(p=>p.toLowerCase()===produto.toLowerCase())){ produtos.push(produto); produtos.sort((a,b)=>a.localeCompare(b)); saveProductsLocal(); renderProdutos(); } render(); e.target.reset(); $('quantidade').value=1; $('funcionario').focus(); toast('Pedido adicionado.'); }catch(err){ console.error(err); toast('Erro ao salvar. Rode o supabase.sql atualizado.'); }finally{ btn.disabled=false; btn.textContent='Adicionar pedido'; } });
  $('produtoForm')?.addEventListener('submit',async(e)=>{ e.preventDefault(); const input=$('novoProduto'); await salvarProduto(input.value); input.value=''; input.focus(); });
  $('busca')?.addEventListener('input',render);
  $('limparTudo')?.addEventListener('click',async()=>{ if(!confirm('Deseja apagar todos os pedidos atuais?'))return; try{ await limparPedidos(); render(); toast('Lista limpa.'); }catch(e){ toast('Erro ao limpar lista.'); } });
  ...
  document.addEventListener('change', e=>{ if(e.target.matches('[data-check-id]')) toggleCheck(e.target.dataset.checkId); });
  document.addEventListener('click', e=>{ const edit=e.target.closest('[data-edit-id]'); if(edit) editarPedido(edit.dataset.editId); const del=e.target.closest('[data-delete-id]'); if(del) excluirPedido(del.dataset.deleteId); const rem=e.target.closest('[data-remove-product]'); if(rem) excluirProduto(rem.dataset.removeProduct); });
}

document.addEventListener('DOMContentLoaded',()=>{ loadLocal(); initEvents(); renderAll(); carregarPedidos(); });
